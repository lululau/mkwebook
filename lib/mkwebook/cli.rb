    ...
    class << self
      ...
      def parse_options!
        ...
        OptionParser.new do |opts|
          ...
          opts.on('-cCONFIG_FILE', '--conf=CONFIG_FILE', 'Specify config file, default is $HOME/.mkwebook.yml, or $HOME/.mkwebook.d/init.yml.') do |config_file|
            @options.config_file = config_file
          end

          opts.on('-iINITIALIZER', '--initializer=INITIALIZER', 'Specify initializer ruby file, default is $HOME/.mkwebook.rb, or $HOME/.mkwebook.d/init.rb.') do |initializer|
      ...
      def default_config_file
        conf = File.expand_path('~/.mkwebook.yml')
        return conf if File.file?(conf)
        conf = File.expand_path('~/.mkwebook.yaml')
        return conf if File.file?(conf)
        conf = File.expand_path('~/.mkwebook.d/init.yml')
        return conf if File.file?(conf)
        conf = File.expand_path('~/.mkwebook.d/init.yaml')
        return conf if File.file?(conf)
      end

      def default_initializer
        conf = File.expand_path('~/.mkwebook.rb')
        return conf if File.file?(conf)
        conf = File.expand_path('~/.mkwebook.d/init.rb')
        return conf if File.file?(conf)
      end
    end
  end
